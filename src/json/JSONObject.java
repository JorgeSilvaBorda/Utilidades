 ...
 * <code>get</code> and <code>opt</code> methods that do type checking and type coercion for you. The opt methods differ from the get methods in that
...
// the deepest nested JSONObject.
     ...
     *
     * If only one value is accumulated that is not a JSONArray, then the result will be the same as using put. But if multiple values are
...
// Shave off trailing zeros and decimal point, if possible.
    ...
    /**
     * Get an optional BigInteger associated with a key, or the defaultValue if there is no such key or if its value is not a number. If the value is
    ...
    /**
     * Get an optional BigDecimal associated with a key, or the defaultValue if there is no such key or if its value is not a number. If the value is
     * a string, an attempt will be made to evaluate it as a number.
     *
     * @param key A key string.
     * @param defaultValue The default.
...
// If klass is a System class then set includeSuperClass to false.
    ...
    /**
     * Creates a JSONPointer using an intialization string and tries to match it to an item within this JSONObject. For example, given a JSONObject
     ...
     * allowing JSON text to be delivered in HTML. In JSON text, a string cannot contain a control character or an unescaped quote or backslash.
     *
	...
	synchronized (w.getBuffer()) {
	    ...
	    return this.write(w, indentFactor, 0).toString().replace("},", "}," + System.getProperty("line.separator"));
	}
    }
